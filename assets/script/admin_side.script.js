$(document).ready(function () {
  ...
  //Buttons for upload, view and delete
  ...
  new DataTable.Buttons(tableData, {
    buttons: [
      ...
      {
        text: '<i class="fas fa-search fa-xs text-primary" title="Search By Date">&nbsp; Search By Date</i>',
        action: function (e, dt, node, config) {
      ...
      {
        text: '<i class="fas fa-trash fa-xs text-danger" title="Delete By Date">&nbsp; Delete By Date</i>',
        action: function (e, dt, node, config) {
          //Trigger the search modal to open
  ...
  $('#deleteDate').on('click', function (e) {
    ...
    var delete_startDate = $('#delete_startDate').val();
    var delete_endDate = $('#delete_endDate').val();

    if (delete_startDate == "" || delete_endDate == "") {
      toastr["error"]("Please fill in all fields.", "ERROR");
    } else {

      $.ajax({
        type: 'POST',
        url: 'controls/deletedata_bydate.ctrl.php',
        data: {
          start_date: delete_startDate,
          end_date: delete_endDate
        },
        success: function (response) {
